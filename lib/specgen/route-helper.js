...
var routeHelper = module.exports = {
  ...
  routeToPathEntry: function(route, classDef,
    ...
    var id = route.method.replace('_prototype.', '_').replace('.', '_');
    ...
    const isPrivate = acls.some(acl => acl.permission !== acl.constructor.ALLOW || acl.principalId !== '$everyone');
    if (isPrivate) {
  ...
  acceptToParameter: function acceptToParameter(route, classDef, typeRegistry, opts) {
    ...
    return function(accepts) {
      ...
      if (paramObject.description === 'Model id' && paramObject.name === 'id' && paramObject.in === 'path' && paramObject.type === 'object') {
...
function createUniqueOperationId(methodName, verb, path, operationIdRegistry) {
  ...
  if (operationIdRegistry[baseId]) {
    ...
    var newId = generateUniqueId(oldEntry.method, baseId, verb, oldEntry.path, operationIdRegistry);
